...
- **BYOK**：使用网站需自带模型 API（LLM + TTS/克隆，默认国内 provider，OpenAI-compatible）。所有 key 仅存浏览器 localStorage，服务端不保存；Fish Audio 免费档也要自己注册 key 使用。
...
## 使用流程（对用户）
1. 在「API Keys」面板填入你的 LLM key 与 TTS/克隆 key。LLM 内置国内模型预设（DeepSeek / 智谱 GLM / Moonshot Kimi / 阿里云百炼 Qwen / OpenCode Go），也可自定义任意 OpenAI-compatible 服务；TTS 可选 **阿里云百炼（Qwen3-TTS 声音复刻，国内直连、稳定）**、**Fish Audio（免费）** 或 **硅基流动 CosyVoice**（详见下方说明）。
...
### TTS / 声音克隆
- **阿里云百炼（Qwen3-TTS 声音复刻，推荐）**：用百炼「通用 API Key」（`sk-ws-` 开头）即可。声音复刻模型为 `qwen3-tts-vc-2026-01-22`，只需 5–20s 参考音频即可生成高相似度定制音色；支持上传音频直接建声纹（无需公网 URL）、长素材分段拟合。没账号可用阿里云账号在 bailian.console.aliyun.com 开通百炼并创建 API Key（新用户一般有免费额度）——同一把 key 也能填到 LLM 栏（选「阿里云百炼 Qwen」）。
- **Fish Audio（免费可用，推荐）**：官方 `s2.1-pro-free` 模型 **$0**（公平使用限流），免费档**包含声纹克隆**。自己注册一个 key 即可：打开 https://fish.audio → 注册/登录 → 控制台 → **API Keys** → 新建一个 key，粘贴到网站「API Keys」面板的 TTS 栏（纯 BYOK，无内置 key）。
...
3. 无需配置任何环境变量：项目为纯 BYOK，所有模型 API key 由用户在前端填写（仅存浏览器 localStorage，服务端不保存任何 key）。
